...
import type { ClearFunction } from "../utils/coroutine-utils";
...
import type { MediaInfo as MediaInfoT } from "./media-loading";
...
export function videoSystem(world: HubsWorld, audioSystem: AudioSystem) {
  ...
  networkedVideoQuery(world).forEach(function (eid: EntityID) {
    ...
    if (hasComponent(world, Owned, eid)) {
      ...
      shouldUpdateVideo = shouldUpdateVideo || src !== currentSrc || autoPlay !== video.autoplay || loop !== video.loop;
